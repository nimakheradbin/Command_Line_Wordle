...
On my/our honor, <Hemant Dhingra> and <Nima Kheradbin>, this 
...
def prepare_game():
    ...
    if len(sys.argv)==1:
        ...
        if arg.isdigit():
            ...
            raise ValueError(INVALID_INPUT)
        elif len(arg)==5 and arg.islower():
            secret_word=arg
...
def get_feedback(secret_word, guessed_word):
    ...
    for i in range(NUM_LETTERS):
        if guessed_word[i]==secret_word[i]:
            ...
            letter_frequency[guessed_word[i]]-=1

    for i in range(NUM_LETTERS):
        if feedback[i] is None:
            if guessed_word[i] in letter_frequency:
                if letter_frequency[guessed_word[i]]>0:
